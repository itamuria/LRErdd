...
mcmc.bin <- function(n.iter, n.burn, dat) {
    ...
    theta <- list(pi.c = rbeta(1, theta.prior$a[1], theta.prior$a[2]), py.nt = rbeta(1, theta.prior$ay.nt[1], theta.prior$ay.nt[2]), py.c0 = rbeta(1, 
...
mcmc.bin.h0 <- function(n.iter, n.burn, dat) {
    ...
    theta <- list(pi.c = rbeta(1, theta.prior$a[1], theta.prior$a[2]), py.nt = rbeta(1, theta.prior$ay.nt[1], theta.prior$ay.nt[2]), py.c = rbeta(1, 
...
EM.gauss <- function(dat, tol = 1e-04, maxit = 1000) {
    ...
    theta <- list(pi.c = rbeta(1, 1, 1), mu.nt = mean(dat$Y) + rnorm(1, 0, sd(dat$Y)), sigma2.nt = runif(1, 0, var(dat$Y)), mu.c0 = mean(dat$Y) + 
        rnorm(1, 0, sd(dat$Y)), sigma2.c0 = runif(1, 0, var(dat$Y)), mu.c1 = mean(dat$Y) + rnorm(1, 0, sd(dat$Y)), sigma2.c1 = runif(1, 0, var(dat$Y)))
...
da.gauss <- function(theta, dat) {
    ...
    den <- (1 - theta$pi.c) * dnorm(dat$Y[dat$Z == 0 & dat$W == 0], theta$mu.nt, sqrt(theta$sigma2.nt)) + theta$pi.c * dnorm(dat$Y[dat$Z == 0 & 
        dat$W == 0], theta$mu.c0, sqrt(theta$sigma2.c0))
...
mcmc.gauss <- function(n.iter, n.burn, dat) {
    ...
    theta <- list(pi.c = rbeta(1, 1, 1), mu.nt = mean(dat$Y) + rnorm(1, 0, 1), sigma2.nt = runif(1, var(dat$Y)/2, 2 * var(dat$Y)), mu.c0 = mean(dat$Y) + 
        rnorm(1, 0, 1), sigma2.c0 = runif(1, var(dat$Y)/2, 2 * var(dat$Y)), mu.c1 = mean(dat$Y) + rnorm(1, 0, 1), sigma2.c1 = runif(1, var(dat$Y)/2, 
        2 * var(dat$Y)))
...
da.gauss.h0 <- function(theta, dat) {
    ...
    den <- (1 - theta$pi.c) * dnorm(dat$Y[dat$Z == 0 & dat$W == 0], theta$mu.nt, sqrt(theta$sigma2.nt)) + theta$pi.c * dnorm(dat$Y[dat$Z == 0 & 
        dat$W == 0], theta$mu.c, sqrt(theta$sigma2.c))
...
mcmc.gauss.h0 <- function(n.iter, n.burn, dat) {
    ...
    theta <- list(pi.c = rbeta(1, 1, 1), mu.nt = mean(dat$Y) + rnorm(1, 0, 1), sigma2.nt = runif(1, var(dat$Y)/2, 2 * var(dat$Y)), mu.c = mean(dat$Y) + 
        rnorm(1, 0, 1), sigma2.c = runif(1, var(dat$Y)/2, 2 * var(dat$Y)))
