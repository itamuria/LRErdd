...
da.gauss2 <- function(theta, dat) {
    ...
    den <- theta$pi[1] * dnorm(dat$Y[dat$Z == 0 & dat$W == 0], theta$mu.nt, sqrt(theta$sigma2.nt)) + theta$pi[3] * dnorm(dat$Y[dat$Z == 0 & dat$W == 
    ...
    den <- theta$pi[2] * dnorm(dat$Y[dat$Z == 1 & dat$W == 1], theta$mu.at, sqrt(theta$sigma2.at)) + theta$pi[3] * dnorm(dat$Y[dat$Z == 1 & dat$W == 
        1], theta$mu.c1, sqrt(theta$sigma2.c1))
...
mcmc.gauss <- function(n.iter, n.burn, dat) {
    ...
    theta <- list(pi = rdirichlet(1, c(1, 1, 1)), mu.nt = mean(dat$Y) + rnorm(1, 0, 1), sigma2.nt = runif(1, var(dat$Y)/2, 2 * var(dat$Y)), mu.at = mean(dat$Y) + 
        rnorm(1, 0, 1), sigma2.at = runif(1, var(dat$Y)/2, 2 * var(dat$Y)), mu.c0 = mean(dat$Y) + rnorm(1, 0, 1), sigma2.c0 = runif(1, var(dat$Y)/2, 
...
da.gauss.h02 <- function(theta, dat) {
    ...
    den <- theta$pi[2] * dnorm(dat$Y[dat$Z == 1 & dat$W == 1], theta$mu.at, sqrt(theta$sigma2.at)) + theta$pi[3] * dnorm(dat$Y[dat$Z == 1 & dat$W == 
        1], theta$mu.c, sqrt(theta$sigma2.c))
...
mcmc.gauss.h02 <- function(n.iter, n.burn, dat) {
    ...
    theta <- list(pi = rdirichlet(1, c(1, 1, 1)), mu.nt = mean(dat$Y) + rnorm(1, 0, 1), sigma2.nt = runif(1, var(dat$Y)/2, 2 * var(dat$Y)), mu.at = mean(dat$Y) + 
        rnorm(1, 0, 1), sigma2.at = runif(1, var(dat$Y)/2, 2 * var(dat$Y)), mu.c = mean(dat$Y) + rnorm(1, 0, 1), sigma2.c = runif(1, var(dat$Y)/2, 
        2 * var(dat$Y)))
    THETA <- matrix(0, n.draws, length(unlist(theta)))
