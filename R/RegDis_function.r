...
rd_pvalue <- function(dataset = last, covariate = "AGE", CT = vec,Nsim = 1000, cutvar = 30,
                      low_lim = NULL, high_lim = NULL)
{

  # if(!is.null(low_lim) & is.null(high_lim))
  # {
  #   dataset <-
  # } else if((is.null(low_lim)*1+is.null(high_lim)*1)==1)
  # {
  #
  # }
...
rd_filtering <- function(dataset = last, covariate = "AGE", num_out = 4, CT = "W",Nsim = 1000, cutvar = 30,
                      low_lim = NULL, high_lim = NULL)
{
   r1 <- dataset$BMI
   min.r <- min(r1)
...
fisher_exact_value <- function(dataset,forcing_bin_var_name="Z",Y_name="dropout",niter=100)
{
      ...
      for(m in 1:M){
        ...
        # if(leny ==2 )
        # {
        #   thyp.m <- abs(mean(Y[Z.m==1])-mean(Y[Z.m==0]))   ### i have to change it to binomial
        # } else if(leny > 2)
        # {
...
#' @export
#'
bw_sens_pajd <- function(dataset = data,forcing_var_name ="S",Y_name ="dropout",niter=1000,
                         covariates = c("sex","HSHumanity","HSTech","HSOther","hsgrade","Y2004"),
                         bandwidth = c(500,1000,1500), cut_value = 15000, whichunder = 1)
      {
        # for each bandwidth
...
#' @export
#'
fep_values <- function(dataset = data,forcing_var_name ="S",Y_name ="dropout",niter=1000,
                       bandwidth = c(500,1000,1500), cut_value = 15000, whichunder = 1)
{
  # for each bandwidth
  len_bw <- length(bandwidth)
...
#' @export
#'
sharp_neyman_bw <- function(dataset = data,forcing_var_name ="S",Y_name ="dropout",niter=1000,
                            bandwidth = c(500,1000,1500), cut_value = 15000, whichunder = 1, cin = 95)
{
  # for each bandwidth
  len_bw <- length(bandwidth)
...
fuzzy_neyman <- function(Y,Wh,Z, cin= 95)
{
      ...
      cov.tau.w.tau.y<- {sum(Y[Z==1]==1 & Wh[Z==1]==1)/sum(Z==1) - m.w1*m.y1}/sum(Z==1)

      Vtau.c <- {1/tau.w^2}*Vneyman.Y + {tau.y^2/tau.w^4}*Vneyman.W - 2*{tau.y/tau.w^3}*cov.tau.w.tau.y
      Vtau.c
      sqrt(Vtau.c)
      ...
      vector <- c(tau.w, tau.y, tau.c,
                  sqrt(Vneyman.W), sqrt(Vneyman.Y), sqrt(Vtau.c),
                  tau.w-zz*sqrt(Vneyman.W),tau.y-zz*sqrt(Vneyman.Y),tau.c-zz*sqrt(Vtau.c),
                  tau.w+zz*sqrt(Vneyman.W),tau.y+zz*sqrt(Vneyman.Y),tau.c+zz*sqrt(Vtau.c)     )
      return(vector)
}
...
#' @export
#'
fuzzy_neyman_bw <- function(dataset = data,forcing_var_name ="S",Y_name ="dropout",niter=1000, W = "W",
                            bandwidth = c(500,1000,1500), cut_value = 15000, whichunder = 1, cin= 95)
{
  # for each bandwidth
  len_bw <- length(bandwidth)
...
#' @export
#'
fuzzy_fep_bw <- function(dataset = data,forcing_var_name ="S",Y_name ="dropout",niter=1000, W = "W",
...
#' @export
#'
sharp_FEP_adj <- function(dataset = data,forcing_var_name ="S",Y_name ="dropout", W_name = "W",
                          covariates = c("sex","HSHumanity","HSTech","HSOther","hsgrade","Y2004"),
                          niter=1000,bandwidth = 1000, cut_value = 15000, whichunder = 1)
{
...
#' @export
#'
sharp_FEP_adj_bw <- function(dataset = data,forcing_var_name ="S",Y_name ="dropout", W_name = "W",
                             covariates = c("sex","HSHumanity","HSTech","HSOther","hsgrade","Y2004"),
                             niter=1000,bandwidth = c(500,1000,5000), cut_value = 15000, whichunder = 1)
{
  ...
  for(h in 1:lenbw)
  {
    ...
    ss <- sharp_FEP_adj (dataset = dataset, forcing_var_name =forcing_var_name,Y_name =Y_name, W_name = W_name,
